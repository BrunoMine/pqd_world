...
-- Tradução intllib
pqd_world.intllib = {}
pqd_world.intllib.S, pqd_world.intllib.NS = dofile(modpath.."/lib/intllib.lua")

-- Configura tradutor opicional
pqd_world.S = pqd_world.intllib.S
pqd_world.NS = pqd_world.intllib.NS
...
-- Gera arquivos de tradução pqd_world.*.tr
do
	...
	for _,file in ipairs(list) do
		...
		if string.match(file, "~") == nil then
			...
			if string.match(file, ".pot") == nil and string.match(file, ".po") then
				...
				-- Novo arquivo
				local new_file = "### Arquivo gerado por pqd_world apartir de "..file.."\n# textdomain: pqd_world\n"
				for en,lang in pairs(en_to_lang) do
					new_file = new_file .. en .. "=" .. lang .. "\n"
				end
				-- Escrever arquivo
				local saida = io.open(modpath.."/locale/pqd_world."..lang_code..".tr", "w")
...
if minetest.get_translator ~= nil then
	s = minetest.get_translator("pqd_world")
else
	s = pqd_world.intllib.S
end

pqd_world.s = function(...)
	local args = { ... }
	if pt_to_en[args[1]] ~= nil then
		return s(pt_to_en[args[1]], unpack(args, 2))
	end
	minetest.log("error", "[pqd_world] String "..dump(args[1]).." nao catalogada")
	return s(...)
end
		...
		if type(args[1]) == "table" then
			...
			for n,a in ipairs(args[1]) do
				if n ~= 1 then -- Não traduz o primeiro
					table.insert(r, pqd_world.S(a))
				else
					table.insert(r, a)
				end
			end
			
			return pqd_world.s(unpack(r))
...
pqd_world.Sfake = function(s) return s end
